...
Converter.prototype.resolveReference = function(base, obj, shouldClone, shouldResolveHttp) {
...
Converter.prototype.convertOperations = function() {
    ...
    for (path in this.spec.paths) {
        pathObject = this.spec.paths[path] = this.resolveReference(this.spec, this.spec.paths[path], true, true);
        this.convertParameters(pathObject); // converts common parameters
        for (method in pathObject) {
            if (HTTP_METHODS.indexOf(method) >= 0) {
                operation = pathObject[method] = this.resolveReference(this.spec, pathObject[method], true, true);
...
Converter.prototype.convertOperationParameters = function(operation) {
    ...
    if (operation.requestBody) {
        param = this.resolveReference(this.spec, operation.requestBody, true, true);
        ...
        if (operation.requestBody.content) {
            ...
            if (data && data.schema && data.schema.$ref && !data.schema.$ref.startsWith('#')) {
                param = this.resolveReference(this.spec, data.schema, true, true);
        ...
        if (content && Object.keys(content).length) {
            ...
            if (contentKey === SUPPORTED_MIME_TYPES.APPLICATION_X_WWW_URLENCODED
                ...
                param.schema = this.resolveReference(this.spec, param.schema, true, true);
...
Converter.prototype.convertParameters = function(obj) {
    ...
    (obj.parameters || []).forEach((param, i) => {
        param = obj.parameters[i] = this.resolveReference(this.spec, param, false, true);
        if (param.in !== 'body') {
            ...
            if (!param.description) {
                const schema = this.resolveReference(this.spec, param.schema, false, true);
...
Converter.prototype.copySchemaProperties = function(obj, props) {
    let schema = this.resolveReference(this.spec, obj.schema, true, true);
    if (!schema) return;
...
Converter.prototype.convertResponses = function(operation) {
    ...
    for (code in operation.responses) {
        response = operation.responses[code] = this.resolveReference(this.spec, operation.responses[code], true, true);
        if (response.content) {
